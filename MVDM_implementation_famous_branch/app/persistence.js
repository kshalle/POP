...
//	endPersisting();   //tell web server that persist protocol ended
    ...
    function persistAnchorAndViewSetRoot(theGraphRoot, shadowGraphRoot) {
        ...
        shadowGraphRoot.backLinkToRootViewSet = theGraphAnchor.rootViewSet.viewSetLinks[0].parentViewSet;
        theGraphAnchor.rootViewSet.viewSetLinks[0].parentViewSet =
            theGraphAnchor.rootViewSet.viewSetLinks[0].parentViewSet.ID;

        //cut off the view set link, that points to the view set of the root element
        shadowGraphRoot.linkToRootElemsViewSet =
            theGraphAnchor.rootViewSet.viewSetLinks[0].childViewSet;
        theGraphAnchor.rootViewSet.viewSetLinks[0].childViewSet =
            theGraphAnchor.rootViewSet.viewSetLinks[0].childViewSet.ID;
    ...
    function visitNextElemAndPersistIt(elem) {
        ...
        if ((elem.viewSet || {}).viewSetLinks) {
            ...
            for (i = 0; i < numLinked; i++) {
                ...
                if (viewSetLink) {
                    viewSetLink.parentViewSet = viewSetLink.parentViewSet.ID;
                    viewSetLink.childViewSet = viewSetLink.childViewSet.ID;
                }
            }
        }
...
// when find one, look up the object with that ID and replace the ID with pointer
    function restoreAnchorAndViewSetRoot( theGraphAnchor ) {
        ...
        theGraphAnchor.rootViewSet.viewSetLinks[0].parentViewSet =
            theObjColl.getByID(theGraphAnchor.rootViewSet.viewSetLinks[0].parentViewSet);
        theGraphAnchor.rootViewSet.viewSetLinks[0].childViewSet =
            theObjColl.getByID(theGraphAnchor.rootViewSet.viewSetLinks[0].childViewSet);
    }
    ...
    function visitNextElemAndRestoreIt( elem ) {
        ...
        if ((elem.viewSet || {}).viewSetLinks) {
            ...
            for (i = 0; i < numLinked; i++) {
                viewSetLink = viewSet.viewSetLinks[i];
                if (viewSetLink) {
                    viewSetLink.parentViewSet = elem.getByID(viewSetLink.parentViewSet);
                    viewSetLink.childViewSet = elem.getByID(viewSetLink.childViewSet);
                }
            }
        }
